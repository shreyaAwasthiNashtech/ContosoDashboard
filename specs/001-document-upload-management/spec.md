...
- **FR-001**: The system MUST allow authenticated employees to upload one or more supported work-related files.
...
- **FR-003**: The system MUST enforce a maximum size of 25 MB per file and provide a clear rejection message when exceeded.
...
- **FR-007**: The system MUST check uploaded files for malware before making them available to users and MUST reject or quarantine files that fail the check.
...
- **FR-014**: The system MUST allow document owners to edit title, description, category, tags, and replace the stored file.
- **FR-015**: The system MUST allow owners to delete their documents and project managers to delete documents in their projects after confirmation; deletion is permanent for this release.
- **FR-016**: The system MUST allow document owners to share documents with selected users or teams and show shared documents in recipients' Shared with Me view.
...
- **FR-021**: The system MUST enforce existing role and project-membership rules at every document access boundary, including list, search, preview, download, update, delete, share, and task attachment operations.
...
- Malware scanning in the offline MVP will use a clearly defined local validation boundary; integration with a production scanning service is future work.
...
- **[NEEDS CLARIFICATION: Offline malware-check behavior]**: What local validation or quarantine behavior should the MVP use when no malware-scanning service is available, and should uploads remain unavailable when that check cannot run?
- **[NEEDS CLARIFICATION: Team sharing scope]**: Which existing team or department relationships define a share target, and which roles may share with an entire team?
- **[NEEDS CLARIFICATION: Multiple-file upload outcome]**: When one file in a multi-file upload fails validation or storage, should the valid files still be committed or should the entire submission fail?
